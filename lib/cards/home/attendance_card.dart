...
class MRAttendanceCard extends ConsumerWidget {
  ...
  Widget _selfiePreview(String path) {
          ...
          children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildAttendanceStep(
                                ...
                                description:
                                    'Each selfie is saved with exact time',
                              ),
                              const SizedBox(height: 16),
                              _buildAttendanceStep(
                                ...
                                title: 'Verification',
                                description:
                                    'Used for security and company policy compliance',
                              ),
                              const SizedBox(height: 16),
                              _buildAttendanceStep(
                                ...
                                title: 'Complete Day',
                                description:
                                    'Check out with a selfie to finish your record',
                              ),
            ...
            if (attendance == null || !attendance.isCheckedIn) ...[
                ...
                onPressed: () async {
                  ...
                  if (file == null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Selfie required to Check In.'),
                      ),
                    );
                    return;
                  }
                  try {
                    await notifier.checkIn(photoPath: file.path);
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                  ...
                  child: Column(
                    ...
                    children: [
                      ClipRRect(
                        ...
                        child: _selfiePreview(attendance.checkInPhotoPath!),
                      ),
                      const SizedBox(height: AppSpacing.sm),
                      Text(
              ...
              ElevatedButton.icon(
                ...
                onPressed: () async {
                  ...
                  if (file == null) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Selfie required to Check Out.'),
                      ),
                    );
                    return;
                  }
                  try {
                    await notifier.checkOut(photoPath: file.path);
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
              ...
              if (attendance.checkOutPhotoPath != null)
                ClipRRect(
                  borderRadius: AppBorderRadius.mdRadius,
                  child: _selfiePreview(attendance.checkOutPhotoPath!),
                ),
            ],
          ],
        ),
