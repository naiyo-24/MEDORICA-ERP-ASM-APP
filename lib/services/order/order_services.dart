...
class OrderServices {
  ...
  Future<List<Order>> fetchOrdersByAsmId(String asmId) async {
    try {
      final response = await _dio.get(ApiUrl.orderAsmGetByAsmId(asmId));
  ...
  Future<Order> createOrder({
    ...
    try {
      ...
      final response = await _dio.post(
        ApiUrl.orderAsmPostByAsmId(asmId),
        data: formData,
  ...
  Future<Order> updateOrder({
    ...
    try {
      ...
      final response = await _dio.put(
        ApiUrl.orderAsmUpdateByOrderId(orderId),
  ...
  Future<void> deleteOrderById(String orderId) async {
    try {
      await _dio.delete(ApiUrl.orderAsmDeleteByOrderId(orderId));
    } on DioException catch (error) {
      throw Exception(_extractErrorMessage(error));
    } catch (_) {
